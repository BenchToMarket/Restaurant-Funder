...
class UPME {
    ...
    function display($args=array()) {
        ...
        if (!$group) {
            if ($id == null && !is_user_logged_in()) {
                ...
                return $this->view_profile($id, $width, $view, $group, $show_stats, $show_social_bar, $use_in_sidebar, $users_per_page, null, $role, $recent_posts);
            }
        }
        ...
        if ($group) {
            if (!is_user_logged_in() && !$this->guests_can_view()) {
                ...
                return $this->view_profile($id, $width, $view, $group, $show_stats, $show_social_bar, $use_in_sidebar, $users_per_page, $hide_until_search, $role, $recent_posts);
            }
        }
    }
    ...
    function display_mini_profile($args=array()) {
        ...
        $display.= '<a href="' . $this->profile_link($id) . '">' . $this->pic($this->logged_in_user, 50) . '</a>';
    ...
    function search($args=array()) {
        ...
        $display.='<form action="" method="post" id="upme_search_form" class="upme-search-form upme-clearfix">';
    ...
    function view_profile($id=null, $width=null, $view=null, $group=null, $show_stats=null, $show_social_bar=null, $use_in_sidebar=null, $users_per_page=null, $hide_until_search=null, $role=null, $recent_posts=null) {
        ...
        if (!$hide_until_search) {
            ...
            if ($users) {
                ...
                foreach ($users as $id) {
                    ...
                    if ('compact' == $view) {
                        $display .= '<div class="' . $pic_class . '"><a href="' . $this->profile_link($id) . '">' . $this->pic($id, 50) . '</a></div>';
                    } else {
                        $display .= '<div class="' . $pic_class . '">' . $this->pic($id, 50) . '</div>';
                    }
    ...
    function edit_profile_fields($id, $width=null, $sidebar_class=null) {
        ...
        if ($this->can_edit_profile($this->logged_in_user, $id)) {
            ...
            <input type="hidden" id="upme-edit-usr-login" value="' . $user_info->user_login . '" />
        ...
        if (get_option('users_can_register') == '1') {
            ...
            if (!isset($display_errors['status']) || (isset($display_errors['status']) && ("error" == $display_errors['status']))) {
                ...
                if (isset($_POST['upme-register']) && $_POST['display_name'] != '') {
                 ...
                 /* mine.    $display .= __('Your display name will appear here.', 'upme'); */
					$display .= __('Start here for Investment details...', 'upme');
                }
    ...
    function show_login_form($register_link_status, $register_text, $custom_register_url, $forgot_link_status, $forgot_text, $custom_forgot_url, $sidebar_class=null, $redirect_to=null) {
        ...
        $separator_text = '';
        if ('yes' == $register_link_status && 'yes' == $forgot_link_status) {
            $separator_text = ' | ';
        }
    ...
    function upme_load_edit_form_scripts() {
        ...
        if (!wp_script_is('upme_fancy_box')) {
        ...
        if (!wp_style_is('upme_fancy_box_styles')) {
    ...
    function upme_show_widget_login_form($widget_settings, $sidebar_class=null, $redirect_to=null) {
        ...
        if ($widget_settings['display-register-link']) {
        ...
        $link_separator = '';
        if (!empty($register_link) && !empty($forgot_pass)) {
    ...
    function upme_sidebar_mini_profile($widget_settings = array()) {
        ...
        $display.= '<a href="' . $this->profile_link($this->logged_in_user) . '">' . $this->pic($this->logged_in_user, 50) . '</a>';
        ...
        if (is_user_logged_in ()) {
            ...
            $display .= '<div class="upme-field-edit upme-widget-profile-button">
                        <a href="' . $link . '" class="' . $class . '">' . $link_text . '</a>&nbsp;' . do_shortcode('[upme_logout wrap_div="false" class="upme-widget-button-alt" user_id="' . $this->logged_in_user . '"]') . '</div>
            </div>';
        }
